...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post('/api/users/:id/custom-analysis', isAuthenticated, async (req, res) => {
    try {
  ...
  app.get('/api/users/:id/unit-assignments', isAuthenticated, async (req, res) => {
    const userId = parseInt(req.params.id);
    if (isNaN(userId)) {
      return res.status(400).json({ message: 'Invalid user ID' });
    }
  ...
  app.post('/api/users/:id/unit-assignments', isAuthenticated, async (req, res) => {
    const userId = parseInt(req.params.id);
    if (isNaN(userId)) {
      return res.status(400).json({ message: 'Invalid user ID' });
    }
    ...
    try {
      ...
      if (assignmentType === 'PRIMARY') {
        ...
        const existingPrimary = allAssignments.find(a =>
          a.assignmentType === 'PRIMARY' && a.id !== assignment.id
        );
  ...
  app.patch('/api/users/:userId/unit-assignments/:assignmentId', isAuthenticated, async (req, res) => {
    ...
    try {
      ...
      if (assignmentType === 'PRIMARY') {
        ...
        const otherPrimary = assignments.find(a =>
          a.assignmentType === 'PRIMARY' && a.id !== assignmentId
        );
  ...
  app.delete('/api/users/:userId/unit-assignments/:assignmentId', isAuthenticated, async (req, res) => {
    const userId = parseInt(req.params.userId);
    const assignmentId = parseInt(req.params.assignmentId);
    ...
    try {
      ...
      if (assignmentToEnd.assignmentType === 'PRIMARY') {
        ...
        const potentialNewPrimary = assignments.find(a =>
          a.id !== assignmentId && a.assignmentType !== 'PRIMARY'
        );
  ...
  app.post('/api/events/:id/add-participants', isAuthenticated, async (req, res) => {
    try {
      const eventId = parseInt(req.params.id);
      ...
      try {
        ...
        const notificationPromises = newParticipants.map(async (participantId: number) => {
          // Create notification
  ...
  app.get('/api/aars/event/:eventId', isAuthenticated, async (req, res) => {
    try {
      ...
      const unitParticipated = Array.isArray(event.participatingUnits) &&
        event.participatingUnits.some((unitId: number) => accessibleUnitIds.includes(unitId));
  ...
  app.patch('/api/units/:id', isAuthenticated, async (req, res) => {
    try {
      ...
      if (parentId !== undefined) updates.parentId = parentId === null ? null : Number(parentId);
  ...
  app.post('/api/units/:unitId/prompt-analysis', isAuthenticated, async (req, res) => {
    try {
      const { unitId } = req.params;
      const { prompt } = req.body;
      const numericUnitId = parseInt(unitId);
