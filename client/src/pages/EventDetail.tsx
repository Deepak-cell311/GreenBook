...
const addParticipantsSchema = addParticipantsInputSchema.transform((data) => ({
  participantIds: data.participantIds.split(',').map(id => parseInt(id.trim()))
}));
...
export default function EventDetail() {
  ...
  useEffect(() => {
    if (event) {
      editEventForm.reset({
        ...
        // Step dates
        step1Date: event.step1Date ? new Date(event.step1Date).toISOString().split('T')[0] : '',
        step2Date: event.step2Date ? new Date(event.step2Date).toISOString().split('T')[0] : '',
        step3Date: event.step3Date ? new Date(event.step3Date).toISOString().split('T')[0] : '',
        step4Date: event.step4Date ? new Date(event.step4Date).toISOString().split('T')[0] : '',
        step5Date: event.step5Date ? new Date(event.step5Date).toISOString().split('T')[0] : '',
        step6Date: event.step6Date ? new Date(event.step6Date).toISOString().split('T')[0] : '',
        step7Date: event.step7Date ? new Date(event.step7Date).toISOString().split('T')[0] : '',
        step8Date: event.step8Date ? new Date(event.step8Date).toISOString().split('T')[0] : ''
      });
    }
  }, [event, editEventForm]);
  ...
  const addUnitMutation = useMutation({
    mutationFn: async (formData: AddUnitFormValues) => {
      const res = await apiRequest("POST", `/api/events/${eventId}/add-unit`, formData);
      return await res.json();
    },
    onSuccess: () => {
  ...
  const completeEventMutation = useMutation({
    mutationFn: async () => {
      const res = await apiRequest("PATCH", `/api/events/${eventId}`, { step: 8 });
      return await res.json();
    },
    onSuccess: () => {
  ...
  const generateAIAnalysis = async () => {
    try {
      ...
      toast({
        ...
        description: "There was an error generating the GreenBookAAR analysis. Please try again.",
        variant: "destructive"
      });
    ...
    <div className="mx-auto max-w-5xl p-4 md:p-6 space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
        <div>
          ...
          <p className="text-muted-foreground">
            Step {event.step}: {Object.entries(TrainingSteps)[event.step - 1][1]}
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="md:col-span-2">
          ...
          <CardContent className="space-y-4">
            <div className="flex flex-col space-y-1">
              ...
              <div className="flex items-center">
                ...
                {event.isMultiDayEvent && event.endDate && (
                  <span> - {format(new Date(event.endDate), 'MMMM d, yyyy')}</span>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col space-y-1">
              <span className="text-sm font-medium text-muted-foreground">Participating Units</span>
              <div className="flex items-start">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Track the 8-step training model progress</CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  <Card key={aar.id}>
                    <CardHeader>
                      <CardTitle className="text-base">
                        AAR from {units?.find(u => u.id === aar.unitId)?.name || `Unit ${aar.unitId}`}
                      </CardTitle>
                      <CardDescription>
                        Submitted {format(new Date(aar.createdAt), 'MMM d, yyyy')}
                      </CardDescription>
                    </CardHeader>
                    <CardContent>
                      <div className="space-y-4">
                        <div>
                          <h4 className="font-medium mb-2">Sustains ({aar.sustainItems?.length || 0})</h4>
                        ...
                        <div>
                          <h4 className="font-medium mb-2">Improves ({aar.improveItems?.length || 0})</h4>
                          <ul className="list-disc pl-5 space-y-1">
                        ...
                        <div>
                          <h4 className="font-medium mb-2">Action Items ({aar.actionItems?.length || 0})</h4>
                          <ul className="list-disc pl-5 space-y-1">
      ...
      <Dialog open={showAddParticipantsDialog} onOpenChange={setShowAddParticipantsDialog}>
        <DialogContent>
          ...
          <Form {...addParticipantsForm}>
            <form onSubmit={addParticipantsForm.handleSubmit(onAddParticipantsSubmit)} className="space-y-4">
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <div className="mt-2">
                      <p className="text-sm text-muted-foreground mb-2">Selected participants:</p>
                      {field.value ? (
                        <div className="flex flex-wrap gap-2">
                          {field.value.split(',').map(id => id.trim()).filter(id => id).map(id => {
                            ...
                            return (
                              ...
                              >
                                {user ? `${user.rank} ${user.name}` : `User ${id}`}
                                <button
                                  type="button"
                                  onClick={() => {
              ...
              <DialogFooter>
                ...
                <Button type="submit" disabled={addParticipantsMutation.isPending}>
      ...
      <Dialog open={showAddUnitDialog} onOpenChange={setShowAddUnitDialog}>
        <DialogContent>
          ...
          <Form {...addUnitForm}>
            <form onSubmit={addUnitForm.handleSubmit(onAddUnitSubmit)} className="space-y-4">
              <FormField
      ...
      <Dialog open={showEditDialog} onOpenChange={setShowEditDialog}>
        <DialogContent className="sm:max-w-[800px] max-h-[80vh] overflow-y-auto">
          ...
          <Form {...editEventForm}>
            <form onSubmit={editEventForm.handleSubmit(onEditEventSubmit)} className="space-y-6">
              <div className="space-y-4">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Textarea rows={3} {...field} placeholder="List equipment, facilities, and other resources needed" />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <Label>Notes</Label>
                          <FormControl>
                            <Textarea rows={2} {...field} placeholder="Planning notes" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <Label>Notes</Label>
                          <FormControl>
                            <Textarea rows={2} {...field} placeholder="Trainer preparation notes" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <Label>Notes</Label>
                          <FormControl>
                            <Textarea rows={2} {...field} placeholder="Site reconnaissance notes" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <Label>Notes</Label>
                          <FormControl>
                            <Textarea rows={2} {...field} placeholder="Order and instructions notes" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <Label>Notes</Label>
                          <FormControl>
                            <Textarea rows={2} {...field} placeholder="Rehearsal notes" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <Label>Notes</Label>
                          <FormControl>
                            <Textarea rows={2} {...field} placeholder="Execution notes" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <Label>Notes</Label>
                          <FormControl>
                            <Textarea rows={2} {...field} placeholder="Evaluation notes" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem>
                          <Label>Notes</Label>
                          <FormControl>
                            <Textarea rows={2} {...field} placeholder="Retraining notes" />
                          </FormControl>
                          <FormMessage />
                        </FormItem>
                      )}
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                    ...
                    <div className="space-y-1 leading-none">
                      ...
                      <p className="text-sm text-muted-foreground">
                        Send notification to participants when updating this event
                      </p>
                    </div>
                  </FormItem>
                )}
      ...
      <Dialog open={showCompleteDialog} onOpenChange={setShowCompleteDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              This will mark the event as completed and advance it to the final step.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <p>Are you sure you want to mark this event as complete? This action can't be undone.</p>
          </div>
          <DialogFooter>
            <Button
              type="button"
          ...
          {aiAnalysisLoading ? (
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {aiAnalysis.frictionPoints.map((point: any, index: number) => (
                    <Card key={index}>
                      <CardContent className="p-4">
                        <div className="flex justify-between items-start">
                          <div>
            ...
            <div className="flex flex-col items-center justify-center py-8">
              ...
              <p className="text-sm text-muted-foreground text-center">
                Click to analyze all AARs for this event using GreenBookAAR's AI capabilities
              </p>
            </div>
          )}
          <DialogFooter>
