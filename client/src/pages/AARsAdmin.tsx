...
export default function AARsAdmin() {
  ...
  // Fetch all AARs
  const { data: allAars = [], isLoading, refetch } = useQuery<AAR[]>({
  ...
  const handleDelete = (aarId: number) => {
    if (confirm("Are you sure you want to delete this AAR? This cannot be undone.")) {
  ...
  const getTotalItems = (aar: AAR) => {
    const sustainCount = Array.isArray(aar.sustainItems) ? aar.sustainItems.length : 0;
    const improveCount = Array.isArray(aar.improveItems) ? aar.improveItems.length : 0;
    const actionCount = Array.isArray(aar.actionItems) ? aar.actionItems.length : 0;
                      ...
                      <TableCell>
                        <div className="flex space-x-2">
                          <Badge variant="outline" className="bg-green-50">
                            Sustain: {Array.isArray(aar.sustainItems) ? aar.sustainItems.length : 0}
                          </Badge>
                          <Badge variant="outline" className="bg-amber-50">
                            Improve: {Array.isArray(aar.improveItems) ? aar.improveItems.length : 0}
                          </Badge>
                          <Badge variant="outline" className="bg-blue-50">
                            Action: {Array.isArray(aar.actionItems) ? aar.actionItems.length : 0}
                          </Badge>
                        </div>
                      </TableCell>
