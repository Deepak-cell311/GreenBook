...
import { Download, Loader2, BarChart2, Calendar, Users, PieChart as PieChartIcon } from "lucide-react";
...
export default function Analytics() {
  ...
  // Events by step
  const eventsByStep = [
    { name: "Risk Assessment", count: events.filter((e: any) => e.step === 1).length },
    { name: "Planning", count: events.filter((e: any) => e.step === 2).length },
    { name: "Preparation", count: events.filter((e: any) => e.step === 3).length },
    { name: "Rehearsal", count: events.filter((e: any) => e.step === 4).length },
    { name: "Execution", count: events.filter((e: any) => e.step === 5).length },
    { name: "AAR", count: events.filter((e: any) => e.step === 6).length },
    { name: "Retraining", count: events.filter((e: any) => e.step === 7).length },
    { name: "Certification", count: events.filter((e: any) => e.step === 8).length },
  ];
  ...
  const eventsByMonth = Array(12).fill(null).map((_, i) => ({
    name: new Date(0, i).toLocaleString('default', { month: 'short' }),
    count: eventsThisYear.filter((e: any) => 
                  ...
                  <CardContent>
                    <div className="space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="border rounded-md p-3">
                          <div className="text-sm text-muted-foreground">AARs Submitted</div>
                          <div className="text-2xl font-bold mt-1">{aars.length}</div>
                        </div>
                        <div className="border rounded-md p-3">
                        ...
                        <div className="border rounded-md p-3">
                          ...
                          <div className="text-2xl font-bold mt-1">
                            {events.filter((e: any) => e.step < 8 && !e.isDeleted).length}
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-semibold mb-2">Events by Stage</h3>
                        <div className="space-y-2">
                          {eventsByStep.map((step, idx) => (
                            <div key={idx} className="flex items-center">
                              ...
                              <div className="flex-1">
                                <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                                  <div 
                                    ...
                                    style={{ 
                                      width: `${Math.max(
                                        (step.count / Math.max(...eventsByStep.map(s => s.count), 1)) * 100, 
                                        step.count > 0 ? 5 : 0
                                      )}%` 
                                    }}
                                  ></div>
                              ...
                              <div className="w-8 text-right text-xs font-medium">{step.count}</div>
                            </div>
                          ))}
                        </div>
                      </div>
                  ...
                  <CardContent>
                    {aars.length === 0 ? (
                      <div className="flex flex-col items-center justify-center h-64">
                        ...
                        <p className="text-muted-foreground">No AAR data available</p>
                      </div>
                    ) : (
                      <>
                        <div className="h-64">
                        ...
                        <div className="flex justify-around text-center mt-4">
                          <div>
                            <div className="text-2xl font-bold">{totalSustainItems}</div>
                            <div className="text-sm text-muted-foreground">Sustain Items</div>
                          </div>
                          <div>
                            <div className="text-2xl font-bold">{totalImproveItems}</div>
                            <div className="text-sm text-muted-foreground">Improve Items</div>
                          </div>
                        </div>
                      </>
                    )}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Card className="md:col-span-2">
                  ...
                  <CardContent>
                    ...
                    <div className="space-y-4">
                      <div>
                        <h3 className="text-sm font-semibold mb-2">Sustain Trends</h3>
                        <div className="space-y-2">
                          <div className="flex items-center">
                            <Badge className="mr-2 bg-green-100 text-green-800 hover:bg-green-100">75%</Badge>
                            <div>Strong squad leader performance mentioned in majority of AARs</div>
                          </div>
                          <div className="flex items-center">
                            <Badge className="mr-2 bg-green-100 text-green-800 hover:bg-green-100">68%</Badge>
                            <div>Effective communication during execution phases</div>
                          </div>
                          <div className="flex items-center">
                            <Badge className="mr-2 bg-green-100 text-green-800 hover:bg-green-100">55%</Badge>
                            <div>Medical treatment procedures consistently followed correctly</div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        <h3 className="text-sm font-semibold mb-2">Improve Trends</h3>
                        <div className="space-y-2">
                          <div className="flex items-center">
                            <Badge className="mr-2 bg-amber-100 text-amber-800 hover:bg-amber-100">82%</Badge>
                            <div>Radio communication issues during night operations</div>
                          </div>
                          <div className="flex items-center">
                            <Badge className="mr-2 bg-amber-100 text-amber-800 hover:bg-amber-100">64%</Badge>
                            <div>Land navigation skills need improvement</div>
                          </div>
                          <div className="flex items-center">
                            <Badge className="mr-2 bg-amber-100 text-amber-800 hover:bg-amber-100">43%</Badge>
                            <div>Pre-operation equipment checks inconsistently performed</div>
                          </div>
                        </div>
                      </div>
                    </div>
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <BarChart 
                          data={[
                            ...
                            { name: 'Call for Fire', initial: 40, retrained: 80 },
                          ]}
                          layout="vertical"
                        >
                          <CartesianGrid strokeDasharray="3 3" />
                          ...
                          <Tooltip />
                          <Legend />
                          <Bar dataKey="initial" name="Initial Training" fill="hsl(var(--chart-2))" />
                          <Bar dataKey="retrained" name="After Retraining" fill="hsl(var(--chart-3))" />
                        </BarChart>
                      </ResponsiveContainer>
                    </div>
                  </CardContent>
                ...
                {veniceAnalysis ? (
                  ...
                  <Card>
                    <CardContent className="flex flex-col items-center justify-center p-12">
                      <PieChartIcon className="h-12 w-12 text-muted-foreground/40 mb-4" />
                      <h3 className="text-lg font-medium mb-2">Venice AI Analysis Unavailable</h3>
                      <p className="text-muted-foreground text-center max-w-md">
                        Venice AI automatic analysis is not available at this time. This could be due to insufficient
                        AAR data or a temporary system issue. However, you can still use the prompt feature above
                        to ask specific questions about your AAR data.
                      </p>
                    </CardContent>
                  </Card>
                )}
