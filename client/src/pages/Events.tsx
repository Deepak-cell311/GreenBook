...
export default function Events() {
  ...
  const getStatusBadge = (step: number) => {
    if (step < 5) return <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100">Preparation</Badge>;
    if (step === 5) return <Badge className="bg-amber-100 text-amber-800 hover:bg-amber-100">Execution</Badge>;
    if (step === 6) return <Badge className="bg-green-100 text-green-800 hover:bg-green-100">AAR</Badge>;
    if (step === 7) return <Badge className="bg-purple-100 text-purple-800 hover:bg-purple-100">Retraining</Badge>;
    if (step === 8) return <Badge className="bg-gray-100 text-gray-800 hover:bg-gray-100">Completed</Badge>;
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredEvents.map((event: any) => (
                <Card key={event.id} className={event.isDeleted ? "opacity-70" : ""}>
                  <CardHeader className="pb-3">
                    <div className="flex justify-between items-start">
                      <div>
                        <CardTitle className="text-lg">{event.title}</CardTitle>
                        <CardDescription>
                          Step {event.step}: {trainingStepInfo[event.step - 1]?.name}
                        </CardDescription>
                      </div>
                      <DropdownMenu>
